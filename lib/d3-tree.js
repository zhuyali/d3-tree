...
export default class D3Tree {
  ...
  renderTree() {
    ...
    var image = node.append('image')
      .attr('xlink:href', d => {
        this.getImageSize(d.data.image)
        return d.data.image;
      })
      .attr('height', d => {
      ...
      .attr('width', d => {
        this.getImageSize(d.data.image).then(size => {
          if (size.width > size.height) {
            return this.imageHeight
          }
        })
      });
    // this.options.isMobile ? '' : image.attr('width', d => { return this.imageHeight; });
  ...
  update(data) {
    ...
    // this.options.isMobile ? '' : this.transition.selectAll('image').attr('width', d => { return this.imageHeight; });
